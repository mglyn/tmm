...
\paragraph{Seed-wise Stability.}
Table~\ref{tab:seedwise} reports the completed three-seed reruns for the learned router.
The routed system varies only within 0.08 points on ChartQA, confirming that the final routed result is stable rather than an artifact of a favorable random seed.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Seed} & \textbf{Val Acc.} & \textbf{Macro-F1} & \textbf{ChartQA} & \textbf{Elapsed (s)} \\
\midrule
42 & 69.26 & 36.48 & 82.56 & 720.62 \\
3407 & 69.38 & 36.56 & 82.64 & 712.21 \\
2025 & 69.01 & 38.57 & 82.64 & 720.80 \\
\midrule
\textbf{Mean $\pm$ Std} & --- & --- & \textbf{82.61 $\pm$ 0.04} & 717.88 \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Seed-wise router reruns on ChartQA.}
\label{tab:seedwise}
\end{table}
...
Table~\ref{tab:component_detail} restates the ablation results from the perspective of absolute accuracy drop relative to the full routed system.
This view is useful because it separates two different questions: which parts are required to train a strong expert, and which parts are required to turn that expert into the final deployed system.
Under this perspective, adapter inheritance remains the dominant training component, while routing provides the final but comparatively small deployment-side gain.
...
\midrule
\textit{Full \method{}+\sar{}} & \textit{82.60} & --- \\
\midrule
w/o Router & 82.12 & $-0.48$ \\
w/o Adapter Inheritance & 77.32 & $-5.28$ \\
w/o Task Staging & 78.64 & $-3.96$ \\
w/o CRS & 79.48 & $-3.12$ \\
w/o Difficulty Ordering & 80.84 & $-1.76$ \\
\midrule
Standard LoRA SFT & 76.01 & $-6.59$ \\
\bottomrule
\end{tabular}
\vspace{4pt}
\caption{Contribution of each component measured as the accuracy drop relative to the full routed system.}
...
Table~\ref{tab:task_profile} compares curriculum training against flat no-curriculum SFT on the five internal task families.
...
\begin{tabular}{lcc}
\toprule
\textbf{Task Type} & \textbf{\method{}} & \textbf{Flat SFT} \\
\midrule
