...
The framework is supported by \textbf{Code-as-Reasoning Supervision (\crs{})}, which teaches explicit intermediate computation through executable Python traces, and \textbf{Difficulty-Aware Curriculum Data Selection (\dacds{})}, which ranks training instances using trace-derived reasoning complexity.
...
Second, \textbf{Difficulty-Aware Curriculum Data Selection (\dacds{})} ranks each sample by trace-derived reasoning complexity, enabling stable easy-to-hard progression without manual annotation.
...
Pix2Struct~\cite{lee2023pix2struct} learns screen understanding through pretraining, MatCha~\cite{liu2023matcha} injects mathematical reasoning and chart derendering, UniChart~\cite{masry2023unichart} designs chart-specific pretraining tasks, DePlot~\cite{liu2023deplot} converts charts to tables before reasoning, ChartReader~\cite{cheng2023chartreader} unifies chart derendering and comprehension, and ChartLlama~\cite{han2023chartllama} instruction-tunes a multimodal LLM directly on chart data.
Recent chart-focused instruction-tuned models and benchmarks, including ChartAssistant~\cite{meng2024chartassistant}, ChartInstruct~\cite{masry2024chartinstruct}, ChartGemma~\cite{masry2024chartgemma}, and ChartBench~\cite{xu2023chartbench}, further show that chart understanding increasingly requires broad evaluation across chart types, visual patterns, and reasoning formats.
Chart summarization and captioning benchmarks such as Chart-to-Text~\cite{kantharaj2022charttotext} and VisText~\cite{tang2023vistext} also emphasize that chart understanding is not limited to short-answer QA, but includes structured description and semantic explanation.
...
Program-aided methods such as PAL and Program-of-Thoughts externalize computation through generated code~\cite{gao2023pal,chen2022program}, and visual programming systems such as VISPROG and ViperGPT use generated programs to compose vision modules at inference time~\cite{gupta2022visprog,suris2023vipergpt}.
Recent work also explores explicit reasoning supervision in visual settings~\cite{lu2024mathvista,shao2024visual}.
...
Sparse expert routing and adapter composition have been studied in mixture-of-experts and transfer-learning systems~\cite{fedus2022switch,pfeiffer2021adapterfusion,huang2024lorahub}.
Our \sar{} applies the same broad specialization principle to stage-specialized LoRA experts learned via curriculum, allowing inference-time specialization without sacrificing usability.
...
\paragraph{Synthetic Data Generation and Curation.}
Beyond model-side design, recent tooling work has begun to formalize the data-engineering side of synthetic supervision.
For example, DataSmith~\cite{datasmith2026} provides a high-concurrency pipeline for LLM data synthesis, filtering, checkpointing, and quality control.
Although it is not chart-specific, it is relevant to our setting because modern multimodal adaptation increasingly depends on scalable generation-and-curation workflows rather than on handwritten one-off scripts alone.
Our pipeline follows the same broad generate--validate--filter pattern, but specializes the validation layer for chart QA by executing reasoning code, checking answer consistency, and assigning difficulty from observable trace features.
...
\label{sec:dacds}
Manual difficulty annotation is impractical at scale, so we assign difficulty from observable properties of the generated supervision traces.
For each chart-question pair, the data builder records the required operation type, the number of reasoning steps, and the depth of dependency among intermediate quantities.
\dacds{} then maps these features to a five-level score using a weighted rule-based scorer, with higher weights on operation complexity and nested dependencies.
Training samples in each stage are sorted in ascending order of this score.
...
This design avoids treating difficulty as an opaque LLM judgment: the generator creates candidate annotations, but the final ordering is derived from verifiable trace metadata.
...
As illustrated in Fig.~\ref{fig:sar_overview}, \sar{} takes a chart-question pair as input, predicts the most suitable stage expert, and dispatches the query to that expert to produce the final answer.
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{figs/fig14_sar.png}
    \caption{Inference-time routing with \sar{}. Given a chart image and question, the router predicts the most suitable stage expert, which then produces the final answer.}
...
\begin{algorithm}[!t]
\caption{Oracle Label Construction for \sar{}}
\label{alg:oracle_router}
\begin{algorithmic}[1]
\REQUIRE Held-out routing set $\mathcal{R}$, deployable experts $\mathcal{E}=\{E_2,E_3,E_4,E_5\}$
\ENSURE Router training pairs $\{(I,q,y)\}$
\FOR{each chart-question pair $(I,q,a)$ in $\mathcal{R}$}
    \FOR{each expert $E_s \in \mathcal{E}$}
        \STATE Generate prediction $\hat{a}_s = E_s(I,q)$
        \STATE Compute correctness $c_s$ with normalized answer matching
        \STATE Compute fallback error $d_s$ using numeric error and edit distance
    \ENDFOR
    \IF{any expert is correct}
        \STATE $y \leftarrow$ a correct expert selected by question-type priority
    \ELSE
        \STATE $y \leftarrow \arg\min_s d_s$
    \ENDIF
    \STATE Add $(I,q,y)$ to the router training set
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
At the pipeline level, this workflow follows the same general pattern emphasized by recent LLM data-synthesis systems such as DataSmith~\cite{datasmith2026}: seed records are expanded into supervision targets, passed through automated validation and filtering, and retained only after quality control.
In our case, the validators are chart-aware rather than generic, including executable-code checking for reasoning traces and rule-based screening for malformed stage annotations.
...
Unless noted otherwise, results are reported from the primary run. We additionally report three-seed stability for the final routed system.
...
We further test zero-shot transfer to PlotQA~\cite{methani2020plotqa} and a 5K subset of FigureQA~\cite{kahou2017figureqa}.
...
\subsection{Evaluation and Reporting Protocol}
\label{sec:reporting_protocol}
Because this paper extends a conference version with new routing experiments, we distinguish three kinds of evidence.
First, the main ChartQA results use the same local evaluation protocol across our own rows: Qwen2.5-VL-7B zero-shot, standard LoRA SFT, the fixed curriculum expert, oracle routing, and \sar{} are all evaluated on the 2,500-sample ChartQA test split.
These numbers support the central claim that the curriculum and router improve in-domain chart question answering under a comparable protocol.

Second, cross-dataset transfer is reported conservatively.
The available PlotQA and FigureQA diagnostics include routed runs, but their subsets and answer formats are not directly comparable to the ChartQA routing setup.
We therefore use the fixed-expert transfer results from the original completed experiments in Table~\ref{tab:transfer}.
This choice intentionally sacrifices a potentially larger routed number in favor of a cleaner comparison against flat SFT.
For the same reason, FigureQA is always described as a 5K subset rather than as the full benchmark.

Third, internal task metrics are used only as diagnostic evidence.
They are valuable for explaining specialization because each internal task family is aligned with one curriculum stage, but they are easier than external ChartQA and should not be interpreted as headline benchmark results.
We therefore use them in the analysis and appendix to explain which component affects which capability, while keeping ChartQA as the primary external target.

Finally, deployment measurements are reported as properties of the current cached-expert evaluation pipeline, not as optimized serving numbers.
The 325.5 ms dispatch time measures the router-side overhead in this implementation, and the 3.49 GB storage figure reflects keeping four LoRA adapters available with a shared backbone.
These measurements are useful for understanding the practical cost of routing, but they should be read as a reproducible systems baseline rather than a fully engineered inference stack.
...
\end{tabular}
\vspace{4pt}
\caption{Comparison on ChartQA. Our full system improves over standard LoRA SFT by 6.59 points while using only 12K synthetic chart samples. External model numbers are cited from their reported ChartQA settings, while our rows use the same local evaluation protocol.}
\label{tab:sota}
\end{table}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & \textbf{Std. SFT} & \textbf{2D-CL} & \textbf{$\Delta$} & \textbf{Setting} \\
\midrule
ChartQA & 76.01 & 82.12 & +6.11 & in-domain \\
PlotQA & 51.9 & 56.5 & +4.6 & zero-shot transfer \\
FigureQA (5K) & 89.7 & 92.3 & +2.6 & zero-shot transfer \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Cross-dataset transfer of the curriculum-trained fixed expert. We report the conservative fixed-expert transfer setting used in the original experiments, avoiding routed transfer numbers when evaluation protocols differ across datasets.}
\label{tab:transfer}
\end{table}

Table~\ref{tab:transfer} summarizes the conservative transfer evidence beyond ChartQA.
Here we use the fixed curriculum expert rather than routed transfer, because the routed PlotQA and FigureQA diagnostics were produced under dataset-specific evaluation subsets and answer formats.
Even under this stricter reporting choice, the curriculum improves over standard SFT on PlotQA and FigureQA, suggesting that the learned skills are not limited to the in-domain ChartQA protocol.
The transfer gains are smaller than the in-domain gain, which is expected: PlotQA is numerically demanding and FigureQA uses binary synthetic questions, so neither dataset exactly matches the open-ended ChartQA supervision.
...
First, adapter inheritance remains the most important training component: relative to the best single-stage expert, removing it costs 4.80 points.
Second, both task staging and within-stage difficulty ordering matter, confirming that the curriculum is genuinely two-dimensional: removing staging costs 3.48 points, while removing difficulty ordering costs 1.28 points.
...
\subsection{Router Stability and Cross-Model Results}
\begin{table}[!t]
\centering
\footnotesize
\begin{tabular}{lccc}
\toprule
\textbf{Seed} & \textbf{Router Val Acc.} & \textbf{Macro-F1} & \textbf{ChartQA} \\
\midrule
2025 & 69.01 & 38.57 & 82.64 \\
3407 & 69.38 & 36.56 & 82.64 \\
42 & 69.26 & 36.48 & 82.56 \\
\midrule
\textbf{Mean $\pm$ Std} & --- & --- & \textbf{82.61 $\pm$ 0.04} \\
\bottomrule
\end{tabular}
\vspace{4pt}
\caption{Three-seed evaluation of the learned router on ChartQA. Validation metrics are reported for the router classifier, and the final column reports end-to-end routed accuracy.}
\label{tab:seeds}
\end{table}

\begin{figure*}[!t]
    \centering
    \subfloat[Ablation study]{\includegraphics[width=0.47\textwidth]{figs/fig3a_v2_ablation_study.png}\label{fig:exp_ablation}}
    \hfil
    \subfloat[Cross-model generalization]{\includegraphics[width=0.47\textwidth]{figs/fig3b_v2_cross_model.png}\label{fig:exp_models}} \\
    \vspace{4pt}
    \subfloat[Cross-dataset transfer]{\includegraphics[width=0.47\textwidth]{figs/fig3c_v2_cross_dataset.png}\label{fig:exp_transfer}}
    \hfil
    \subfloat[Routing analysis]{\includegraphics[width=0.47\textwidth]{figs/fig3d_v2_routing_analysis.png}\label{fig:exp_routing}}
    \caption{Overview of experimental evidence. The four panels show that each curriculum component contributes positively, the gains transfer across backbones, the learned capability generalizes beyond ChartQA, and stage-aware routing converts stage specialization into inference-time improvement.}
    \label{fig:main_experiments}
\end{figure*}

The three-seed evaluation shows that \method{}+\sar{} reaches 82.61 $\pm$ 0.04 on ChartQA.
The small standard deviation indicates that the final routed result is not an artifact of a favorable random seed.
Router validation macro-F1 is lower than validation accuracy because oracle labels are imbalanced toward Stage 2, which is consistent with the deployment profile analyzed in Section~\ref{sec:routing_analysis}.
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Flat SFT} & \textbf{2D-CL} & \textbf{$\Delta$} \\
\midrule
Qwen2.5-VL-7B & 75.96 & 82.12 & +6.16 \\
Qwen2.5-VL-3B & 72.28 & 76.44 & +4.16 \\
InternVL2-8B & 76.84 & 80.76 & +3.92 \\
\bottomrule
\end{tabular}
\vspace{4pt}
\caption{Cross-model evaluation on ChartQA using the fixed curriculum expert. The proposed training organization improves both smaller and architecturally different backbones.}
\label{tab:models}
\end{table}

Table~\ref{tab:models} confirms that the benefits of \method{} are not tied to a single model family.
...
In particular, the non-Qwen InternVL2-8B result is useful evidence that the curriculum is not simply exploiting quirks of a single multimodal backbone.
...
Because this sample is restricted to failures shared by the best fixed expert and the routed system, it should be interpreted as a diagnosis of the remaining bottlenecks rather than as a complete error comparison against standard SFT.

Table~\ref{tab:error_examples} gives representative shared-failure examples from the same labeled set.
The examples show why the dominant categories are difficult to eliminate with curriculum structure alone.
Counting and ranking errors often require fine-grained perception of all marks in a dense chart; arithmetic errors can arise even when the model identifies a plausible subset of values; and legend or axis confusions propagate into a wrong final answer even if the response format is clean.

\begin{table}[!t]
\centering
\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular}{p{0.43\columnwidth}p{0.14\columnwidth}p{0.16\columnwidth}p{0.18\columnwidth}}
\toprule
\textbf{Question} & \textbf{Gold} & \textbf{Prediction} & \textbf{Error Type} \\
\midrule
How many food items are shown? & 14 & 15 & dense counting \\
What was the 4th most popular emotion? & Inspired & Connected & axis mapping \\
Is the sum of largest two bars greater than the sum of smallest three bars? & No & Yes & multi-step \\
Ratio of the lowest green and blue bars? & 1.2167 & 13:7 & arithmetic \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Representative shared failures where both the best fixed expert and \method{}+\sar{} are incorrect.}
...
\end{tabular}
\vspace{4pt}
\caption{Internal task-wise performance across stage experts. T1 is BLEU, T2--T5 are accuracy (\%). Each stage peaks on the task family it was designed to emphasize; the 100\% scores reflect the limited, format-consistent internal evaluation set.}
...
The largest drop appears on reasoning-heavy questions, where performance falls from 100.0\% to 72.4\% on the internal reasoning benchmark when \crs{} is removed.
...
Table~\ref{tab:difficulty_patterns} groups samples by their question type and reports the mean trace-derived difficulty level.
...
Variance/Std.       & 5.0      & 100 \\
\midrule
\multicolumn{3}{l}{\textit{Overall 1,973 samples across 5 levels; distribution: 10.4\%/33.5\%/31.8\%/14.2\%/10.1\%.}} \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Validation of trace-derived \dacds{} difficulty scores by question type. The assigned levels align monotonically with the mathematical complexity of the operations required to answer each question.}
...
\section{Discussion}
The TMM extension changes the practical interpretation of the curriculum.
In the ACM MM version, stage specialization was mainly an internal empirical observation.
With \sar{}, specialization becomes a deployable inference mechanism that can be evaluated in terms of both accuracy and systems cost.
The current gain over the best fixed expert is modest (+0.48), but the 4.88-point gap between predicted and oracle routing shows that the curriculum itself is not the main bottleneck anymore.
Instead, the limiting factor is how well the deployment policy can identify which expert should answer which question.

A second implication is that the strongest gains come from organizing scarce supervision more effectively rather than from simply scaling data volume.
Across ChartQA, PlotQA, and the FigureQA 5K subset, the reliable fixed-expert results are most consistent with improvements on behaviors that flat SFT tends to entangle: concise answer formatting, explicit arithmetic structure, and task-dependent specialization.
This pattern supports a broader claim of the paper: when a multimodal task couples perception, extraction, and symbolic reasoning, training order can serve as an inductive bias rather than a mere optimization heuristic.

At the same time, the remaining failures clarify what curriculum learning alone does not solve.
Arithmetic and value extraction still dominate the shared error pool, and dense charts continue to expose limitations of frozen visual representations.
Future work should therefore combine stronger routing with better numerical grounding and expert compression, so that the benefits of specialization can be recovered more completely and deployed more economically.
Another immediate direction is to scale the data construction side more systematically.
Frameworks such as DataSmith~\cite{datasmith2026} suggest a practical path toward larger chart-supervision pipelines with built-in concurrency control, failure recovery, and quality filtering, which would make it easier to test whether the advantages of our curriculum persist when synthetic corpora grow from tens of thousands to substantially larger scales.
...
Conservative zero-shot transfer results on PlotQA and a FigureQA 5K subset further suggest that the learned capabilities generalize beyond the training distribution.
...
Across ten scenario categories, business and science are the most common sources, reflecting domains where chart question answering is especially relevant.
Difficulty levels are intentionally skewed toward easier examples, which matches the philosophy of exposing the model to more low-complexity cases before progressively introducing harder ones.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{figs/fig11_dataset_distribution.png}
    \caption{Distribution of synthetic training data. (a) Chart types. (b) Scenario categories. (c) Difficulty levels.}
    \label{fig:dataset_dist}
\end{figure*}

\paragraph{Synthetic Supervision Expansion.}
The curriculum data are expanded from 1,973 chart records into multiple supervision families.
For Stages 1, 2, 3, and 5, each chart contributes one training target; Stage 4 contributes three visual-analysis targets per chart, which is why it contains roughly three times as many samples.
We use the same 90/10 train-validation split across stages, yielding 12,427 training samples and 1,384 validation samples in total.
Table~\ref{tab:data_expansion} summarizes this expansion explicitly so that the reported 12K training scale is auditable.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Stage} & \textbf{Task Family} & \textbf{Train} & \textbf{Val} & \textbf{Total} \\
\midrule
S1 & Description & 1,775 & 198 & 1,973 \\
S2 & Basic VQA & 1,775 & 198 & 1,973 \\
S3 & Reasoning VQA & 1,775 & 198 & 1,973 \\
S4 & Visual Analysis & 5,327 & 592 & 5,919 \\
S5 & Code Generation & 1,775 & 198 & 1,973 \\
\midrule
\textbf{Total} & --- & \textbf{12,427} & \textbf{1,384} & \textbf{13,811} \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Stage-wise expansion from 1,973 synthetic chart records to curriculum supervision samples.}
\label{tab:data_expansion}
\end{table}

\paragraph{Quality-Control Checks.}
The data builder applies lightweight validation before retaining generated supervision.
Reasoning targets are checked by executing the generated Python trace and comparing the result with the expected answer.
Stage annotations are screened for missing images, malformed prompts, empty answers, and invalid difficulty metadata.
For \dacds{}, the stored difficulty level is derived from trace features rather than copied blindly from generation text, which keeps the easy-to-hard ordering tied to auditable properties of the sample.
These checks are intentionally simple, but they make the pipeline reproducible and align with the broader generate--validate--filter pattern discussed in connection with DataSmith.
...
We report only quantities recorded by the completed training run: train/validation sample counts, final losses, and wall-clock time.
...
& \textbf{S1} & \textbf{S2} & \textbf{S3} & \textbf{S4} & \textbf{S5} \\
\midrule
Train Samples & 1,775 & 1,775 & 1,775 & 5,327 & 1,775 \\
Val Samples & 198 & 198 & 198 & 592 & 198 \\
Final Train Loss & 0.6665 & 0.3761 & 0.2742 & 0.1766 & 0.1103 \\
Final Val Loss & 0.7140 & 1.5701 & 0.4098 & 0.7011 & 0.2027 \\
Wall-clock & 3h24m & 2h34m & 2h47m & 8h20m & 3h25m \\
\bottomrule
\end{tabular}
\vspace{4pt}
\caption{Training summary across stages from the completed curriculum run. Stage 4 is the most time-consuming because it contains roughly three times as many samples as the other stages.}
\label{tab:training_dynamics}
\end{table}

\paragraph{Additional Diagnostics.}
We include two compact diagnostics that complement the main experimental section: the routing distribution of the deployed \sar{} module and an internal task-family breakdown for the ablation variants.

\paragraph{ChartQA Routing Profile.}
Table~\ref{tab:routing_profile} reports the stage usage of the final routed ChartQA evaluation.
The router is intentionally conservative: most samples are sent to Stage 2, while a small fraction is redirected to reasoning, visual-analysis, or code-oriented experts.
This behavior matches the accuracy result in Table~\ref{tab:routing}: the current router is useful as a correction layer, but the oracle gap shows that a more discriminative dispatcher could exploit specialization more fully.
...
\begin{tabular}{lcc}
\toprule
\textbf{Selected Expert} & \textbf{\# Samples} & \textbf{Ratio} \\
\midrule
Stage 2: Basic VQA & 2375 & 95.00\% \\
Stage 3: Reasoning VQA & 101 & 4.04\% \\
Stage 4: Visual Analysis & 22 & 0.88\% \\
Stage 5: Code Generation & 2 & 0.08\% \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Predicted stage usage for the final ChartQA routed evaluation.}
\label{tab:routing_profile}
\end{table}

\paragraph{Internal Ablation Breakdown.}
Table~\ref{tab:internal_ablation} further breaks down the ablation study on the five internal task families.
These internal metrics are not substitutes for ChartQA, but they are useful for explaining \emph{why} each component matters.
Removing \crs{} mainly harms reasoning, removing inheritance broadly weakens all downstream capabilities, and removing staging hurts every task because the model loses the intended capability sequence.
...
\begin{tabular}{lccccc}
\toprule
\textbf{Configuration} & \textbf{T1} & \textbf{T2} & \textbf{T3} & \textbf{T4} & \textbf{T5} \\
& Desc. & VQA & Reas. & Vis. & Code \\
\midrule
Full 2D-CL & 25.0 & 24.2 & 100.0 & 81.3 & 100.0 \\
w/o Staging & 18.4 & 21.6 & 78.3 & 68.7 & 84.2 \\
w/o Difficulty & 23.2 & 23.1 & 96.8 & 77.4 & 95.6 \\
w/o CRS & 24.6 & 23.8 & 72.4 & 79.8 & 98.4 \\
w/o Inheritance & 16.2 & 19.4 & 68.6 & 62.1 & 76.8 \\
Flat SFT & 12.8 & 18.2 & 58.4 & 54.6 & 62.4 \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Internal validation breakdown for ablation configurations. T1 is BLEU, while T2--T5 are accuracy (\%).}
\label{tab:internal_ablation}
\end{table}
