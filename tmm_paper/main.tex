...
% TODO-EXP-06: PlotQA / FigureQA rows now reflect the available real runs.
\begin{table}[!t]
\centering
\small
\begin{tabular}{lcccc}
...
PlotQA & --- & --- & 58.10 & SAR result from completed external run \\
FigureQA (5K) & --- & --- & 98.06 & SAR result on 5K QA pairs \\
\bottomrule
\end{tabular}
...
% TODO-EXP-07: Add Std. SFT / Best Stage rows for PlotQA and FigureQA if needed.
Table~\ref{tab:transfer} summarizes the currently finalized transfer evidence beyond ChartQA.
On PlotQA, the routed system reaches 58.10\% accuracy. On FigureQA, which uses synthetic binary chart questions and a different answer format from the open-ended training supervision, \method{}+\sar{} reaches 98.06\% accuracy on a 5K subset.
Together with the in-domain ChartQA result, these experiments support the claim that the curriculum learns transferable chart understanding behavior across multiple chart QA benchmarks.
