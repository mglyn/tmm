...
\begin{figure*}[!t]
    \centering
    \subfloat[Ablation study]{\includegraphics[width=0.32\textwidth]{figs/fig3a_v2_ablation_study.png}\label{fig:exp_ablation}}
    \hfil
    \subfloat[Cross-model generalization]{\includegraphics[width=0.32\textwidth]{figs/fig3b_v2_cross_model.png}\label{fig:exp_models}}
    \hfil
    \subfloat[Cross-dataset transfer]{\includegraphics[width=0.32\textwidth]{figs/fig3c_v2_cross_dataset.png}\label{fig:exp_transfer}}
...
Across ChartQA, PlotQA, and FigureQA, the improvements are most visible on behaviors that flat SFT tends to entangle: concise answer formatting, explicit arithmetic structure, and task-dependent specialization.
...
Table~\ref{tab:seedwise} reports the completed three-seed reruns for the final stage-selection comparison.
The best fixed stage is numerically unchanged across all three seeds, while the routed system varies only within 0.08 points.
This confirms that the final routing gain is small but stable rather than an artifact of a favorable random seed.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Seed 42} & \textbf{Seed 3407} & \textbf{Seed 2025} & \textbf{Mean $\pm$ Std} \\
\midrule
Best single-stage expert & 80.24 & 80.24 & 80.24 & 80.24 $\pm$ 0.00 \\
\textbf{\method{}+\sar{}} & 82.56 & 82.64 & 82.64 & \textbf{82.61 $\pm$ 0.04} \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Seed-wise reruns for the final stage-selection comparison on ChartQA.}
\label{tab:seedwise}
\end{table}

\paragraph{Routing Profile Across Datasets.}
Table~\ref{tab:routing_profile} shows how the learned dispatcher behaves on the three finalized routed evaluations currently available in this draft.
The contrast across datasets is informative.
ChartQA is dominated by Stage 2, PlotQA activates both Stage 2 and Stage 5 more often, and FigureQA is routed overwhelmingly to Stage 4.
This pattern supports the interpretation that the experts are not interchangeable; rather, they respond to the different supervision regimes emphasized during curriculum training.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Dataset} & \textbf{Stage 2} & \textbf{Stage 3} & \textbf{Stage 4} & \textbf{Stage 5} \\
\midrule
ChartQA (2.5K) & 2375 (95.0\%) & 101 (4.04\%) & 22 (0.88\%) & 2 (0.08\%) \\
PlotQA (1K) & 810 (81.0\%) & 10 (1.0\%) & 83 (8.3\%) & 97 (9.7\%) \\
FigureQA (5K) & 80 (1.6\%) & 12 (0.2\%) & 4908 (98.2\%) & 0 (0.0\%) \\
\bottomrule
\end{tabular}}
\vspace{4pt}
\caption{Predicted stage usage across the completed routed evaluations.}
