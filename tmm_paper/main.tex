...
% TODO-EXP-06: PlotQA / ChartBench / OpenCQA baselines are still pending.
\begin{table}[!t]
\centering
\small
\begin{tabular}{lcccc}
...
\midrule
ChartQA & 76.01 & 82.12 & 82.60 & in-domain reference \\
FigureQA (5K) & --- & --- & 98.06 & measured zero-shot transfer \\
\bottomrule
\end{tabular}
\caption{Current transfer results with real measurements available in this draft. ChartQA is listed as the in-domain reference, while FigureQA reports the completed zero-shot transfer run on 5,000 QA pairs. PlotQA, ChartBench, and OpenCQA are still being added.}
\label{tab:transfer}
\end{table}

% TODO-EXP-07: Extend this paragraph after PlotQA / ChartBench / OpenCQA are done.
Table~\ref{tab:transfer} provides the currently completed transfer evidence beyond ChartQA.
On FigureQA, which contains synthetic binary chart questions in a format different from the open-ended training supervision, \method{}+\sar{} reaches 98.06\% accuracy on a 5K subset.
This result supports the claim that the curriculum learns transferable chart understanding behavior, although a fuller cross-dataset picture still requires the remaining benchmarks.
