...
\midrule
ChartQA & 76.01 & 82.12 & 82.60 & +6.59 \\
...
\midrule
\method{} (best single stage) & TBD \\
\method{}+\sar{} & 82.61 $\pm$ 0.04 \\
\bottomrule
\end{tabular}
\caption{Three-seed evaluation on ChartQA for the final stage-selection comparison. We report multi-seed stability for the routed system and reserve the corresponding best single-stage 2D-CL row for the same rerun protocol.}
\label{tab:seeds}
\end{table}
...
The three-seed evaluation for \method{}+\sar{} gives 82.61 $\pm$ 0.04 on ChartQA, indicating that the routing policy is highly stable across random seeds.
Following the reduced protocol adopted for this journal version, the remaining multi-seed rerun is the corresponding best single-stage 2D-CL baseline, which will be refreshed under the same hardware and evaluation pipeline.
...
Table~\ref{tab:efficiency} reports the measurable deployment-side cost of stage routing under the current evaluation pipeline.
Because routing requires a separate frozen-backbone forward pass before expert selection, the present implementation adds 325.5 ms of router-side dispatch time per sample on ChartQA.
Each LoRA expert occupies roughly 873 MB, so storing all four stage adapters requires 3.49 GB compared with 872.6 MB for a single fixed-stage deployment.
...
Router-side dispatch / sample (ms) & --- & 325.5 \\
Adapter storage (MB) & 872.6 & 3490.7 \\
Stored adapters & 1 & 4 \\
Backbone copies & 1 & 1 \\
\bottomrule
\end{tabular}
\caption{Routing efficiency under the current cached-expert evaluation pipeline. We report the measured router-side dispatch cost and the storage required to keep the stage-specialized LoRA bank available.}
